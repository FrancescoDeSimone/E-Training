...
public class TestRichiesteTirocinioInviateServlet {
  ...
  public void test01_SessioneNull() throws ServletException, IOException {
    ...
    when(request
        .getRequestDispatcher("/login.jsp")).thenReturn(dispatcher);

    try {
      new RichiesteTirocinioInviateServlet().doGet(request, response);
  ...
  public void test02_SessioneNotNull() throws ServletException, IOException {
    ...
    when(request
        .getRequestDispatcher("/paginaErrore.jsp?Erore nel connettersi al database.")).thenReturn(dispatcher);
