...
public class TestRichiestaTirocinioServlet {
  ...
  public void test01_SessioneNull() throws ServletException, IOException {
    ...
    try {
      new RichiestaTirocinioServlet().doPost(request, response);
  ...
  @Test
  public void test02_AziendaNull() throws ServletException, IOException {
    sessione.setAttribute("utente", unTirocinante);
    when(request.getSession(false)).thenReturn(sessione);
    when(request.getParameter("label0")).thenReturn(unTirocinio.getObiettivi()[0]);
    when(request.getParameter("azienda")).thenReturn(null);

    when(request.getRequestDispatcher("/inviaRichiesteDiTirocinio.jsp?errore=1"))
  ...
  @Test
  public void test03_TemaNull() throws ServletException, IOException {
    sessione.setAttribute("utente", unTirocinante);
    when(request.getSession(false)).thenReturn(sessione);
    when(request.getParameter("label0")).thenReturn(unTirocinio.getObiettivi()[0]);
    when(request.getParameter("offertaFormativa")).thenReturn(null);
  ...
  @Test
  public void test04_CFUNull() throws ServletException, IOException {
    sessione.setAttribute("utente", unTirocinante);
    when(request.getSession(false)).thenReturn(sessione);
    when(request.getParameter("label0")).thenReturn(unTirocinio.getObiettivi()[0]);
    when(request.getParameter("cfu")).thenReturn(null);
  ...
  @Test
  public void test05_AziendaFailRegex() throws ServletException, IOException {
    sessione.setAttribute("utente", unTirocinante);
    when(request.getSession(false)).thenReturn(sessione);
    when(request.getParameter("label0")).thenReturn(unTirocinio.getObiettivi()[0]);
    when(request.getParameter("azienda")).thenReturn("provaAzienda@@@@mailsbagliata");
    when(request.getParameter("offertaFormativa")).thenReturn(unOfferta.getTema());
  ...
  @Test
  public void test06_TemaFailRegex() throws ServletException, IOException {
    sessione.setAttribute("utente", unTirocinante);
    when(request.getSession(false)).thenReturn(sessione);
    when(request.getParameter("label0")).thenReturn(unTirocinio.getObiettivi()[0]);
    when(request.getParameter("azienda")).thenReturn("provaAzienda@@@@mailsbagliata");
    when(request.getParameter("offertaFormativa")).thenReturn("we");
    when(request.getParameter("cfu")).thenReturn("" + 6);
  ...
  @Test
  public void test07_CFUFailRegex() throws ServletException, IOException {
    sessione.setAttribute("utente", unTirocinante);
    when(request.getSession(false)).thenReturn(sessione);
    when(request.getParameter("label0")).thenReturn(unTirocinio.getObiettivi()[0]);
    when(request.getParameter("azienda")).thenReturn(unAzienda.getEmail());
    when(request.getParameter("offertaFormativa")).thenReturn(unOfferta.getTema());
    when(request.getParameter("cfu")).thenReturn("" + 10);
  ...
  @Test
  public void test08_Noobbiettivi() throws ServletException, IOException {
    sessione.setAttribute("utente", unTirocinante);
    when(request.getSession(false)).thenReturn(sessione);
    when(request.getParameter("azienda")).thenReturn(unAzienda.getEmail());
    when(request.getParameter("offertaFormativa")).thenReturn(unOfferta.getTema());
  ...
  @Test
  public void test09_ObbiettiviRegexFail() throws ServletException, IOException {
    sessione.setAttribute("utente", unTirocinante);
    when(request.getSession(false)).thenReturn(sessione);
    when(request.getParameter("label0")).thenReturn("  ");
    when(request.getParameter("label1")).thenReturn(null);
    when(request.getParameter("azienda")).thenReturn(unAzienda.getEmail());
    when(request.getParameter("offertaFormativa")).thenReturn(unOfferta.getTema());
    when(request.getParameter("cfu")).thenReturn("" + 6);
  ...
  @Test
  public void test10_ParamasOK() throws ServletException, IOException {
    sessione.setAttribute("utente", unTirocinante);
    when(request.getSession(false)).thenReturn(sessione);
    when(request.getParameter("label0")).thenReturn(unTirocinio.getObiettivi()[0]);
    when(request.getParameter("azienda")).thenReturn(unAzienda.getEmail());
    ...
    when(request.getRequestDispatcher(
        "/paginaErrore.jsp?errore=Errore nel " + "salvare la richiesta di tirocinio."))
            .thenReturn(dispatcher);
